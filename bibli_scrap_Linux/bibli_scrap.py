...
class bibli_scrap(bibli) :
    ...
    def alimenter2(self,url,nbmax,nbmaxsave) : #Cette deuxième version de la méthode alimenter considère cette fois-ci un nombre maximale de livre à télécharger. 
            ...
            while i<len(ListeUrls) and nbmax[0]>0 :
                ...
                if (ListeUrls[i][-5:]==".epub" or ListeUrls[i][-4:]==".pdf") and osp.basename(ListeUrls[i]) not in Livres_presents : 
                    self.ajouter(ListeUrls[i])
                    nbmax[0]-=1
                    
                    if (nbmaxsave-nbmax[0])%50==0 : print(f'{nbmaxsave-nbmax[0]} livres téléchargés.')
                    
                i+=1  
       
    def scrap(self,url,profondeur,nbmax,Lurl=set([]),premiereappel=True,nbmaxsave=0,arret=[False]) : #Lurl : liste des sites visités
        if premiereappel : 
        ...
        if reqs.status_code == 200 :
            ...
            if premiereappel : nbmaxsave=nbmax #On enregistre le nbmax rentré au début pour le message à la fin.
            ...
            self.alimenter2(url,nbmax,nbmaxsave)
            ...
            if nbmax[0]>0 and profondeur[0]>0  :
                ...
                for site in Liste_autres_sites : 
                    if (site not in Lurl) and profondeur[0]>0 and nbmax[0]>0 : #On vérifie que le site n'a pas déjà été visité.
                        self.scrap(site,profondeur,nbmax,Lurl,False,nbmaxsave,arret) #On rajoutera le nouveau site visité à Lurl
            elif nbmaxsave-nbmax[0]==0 : 
                print(f"Aucun nouveau livre n'a été trouvé sur {len(Lurl)} page{'s'*(len(Lurl)>1)}.")
                arret[0]=True
            elif len(Lurl)==1 : 
                print(f"Scraping de {nbmaxsave-nbmax[0]} livres sur une page effectué avec succés.")
                arret[0]=True
            else : 
                print(f"Scraping de {nbmaxsave-nbmax[0]} livres sur {len(Lurl)} pages effectué avec succés.")
                arret[0]=True
